...
object AsyncHomework extends App {
    ...
    url match {
        case Some(url) => process(url)
        case None => {
            println("As url will be used default url - http://google.com/")
            process("http://google.com/")
        }
    }

    def process(url: String): Unit = for {
        body <- fetchPageBody(url)
        links <- findLinkUrls(body)
        serverNames <- Future.sequence(links.map(fetchServerName))
    } yield serverNames.map(_.getOrElse("")).filter(_ != "").sorted.distinct.foreach(println)
