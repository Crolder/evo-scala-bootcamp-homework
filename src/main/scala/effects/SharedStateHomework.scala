...
object SharedStateHomework extends IOApp {
  ...
  class RefCache[F[_] : Clock : Monad, K, V](
    ...
    def get(key: K): F[Option[V]] = state.get.map(map => map.get(key).map{ case (_, value) => value })

    def put(key: K, value: V): F[Unit] = for {
      currentTime <- Clock[F].realTime(MILLISECONDS)
      _ <- state.update(currentState => currentState ++ Map((key,(expiresIn.toMillis + currentTime, value))))
    } yield ()
  ...
  object Cache {
    def of[F[_] : Clock, K, V](
      ...
      def cleanExpired(state: Ref[F, Map[K, (Long, V)]]): F[Unit] = for {
        _ <- T.sleep(checkOnExpirationsEvery)
        currentTime <- Clock[F].realTime(MILLISECONDS)
        oldState <- state.get
        _ <- state.set(oldState.filter {case (_, (exTime, _)) => currentTime - exTime < expiresIn.toMillis})
      } yield ()

      for {
        cache <- Ref.of[F, Map[K, (Long, V)]](Map())
        _ <- C.start(cleanExpired(cache).foreverM.void)
      } yield new RefCache(cache, expiresIn)
    }
